...
def parse_zip(blob: bytes):
    ...
    for raw in rows:
        ...
        if not t or (temp is None and rh is None):
            continue
        out.append({'observed_at_utc':t.isoformat().replace('+00:00','Z'),'temperature_c':temp,'relative_humidity_pct':rh})
    if not out:
...
def main():
    ...
    meta={'provider':'DWD CDC 10-minute now','station_id':STATION_ID,'station_name':STATION_NAME,
    ...
    new=[]
    for p in points:
        if p['observed_at_utc'] not in known:
    ...
    status={'ok':True,**meta,'source_url':ZIP_URL,'checked_at_utc':now.isoformat().replace('+00:00','Z'),
            'observed_at_utc':latest['observed_at_utc'],'age_minutes':round(age_min,1),
            'temperature_c':latest.get('temperature_c'),'relative_humidity_pct':latest.get('relative_humidity_pct'),
            'new_points_saved':len(new),'available_points_now':len(points),'archive_file':f'data/observations/{archive.name}'}
