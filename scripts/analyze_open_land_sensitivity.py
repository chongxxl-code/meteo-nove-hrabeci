...
OUT = ROOT / 'data' / 'validation' / 'open-land-sensitivity-2026.json'
STATUS = ROOT / 'data' / 'validation' / 'open-land-sensitivity-status.json'
...
def summarize(rows,predictor,index_name,expected):
    rs=[((r.get(index_name) or {}).get(predictor) or {}).get('pearson_r') for r in rows]
    vals=[float(x) for x in rs if finite(x)]
...
def main():
    ...
    excluded={(x['sample_id'],x['scene_id']) for x in dist.get('disturbed_observations_for_sensitivity_test') or []}
    ...
    filtered={'ndmi':{},'ndvi':{}}
    for idx in ('ndmi','ndvi'):
        for p in predictors: filtered[idx][p]=summarize(rows,p,idx,expected[p])
    comparison={}
    raw_summary=raw.get('summary') or {}
    for idx in ('ndmi','ndvi'):
        comparison[idx]={}
        for p in predictors:
            r=(raw_summary.get(idx) or {}).get(p) or {}; f=filtered[idx][p]
            comparison[idx][p]={
                'raw_median_r':r.get('median_pearson_r'),
                'filtered_median_r':f.get('median_pearson_r'),
                'delta_median_r':round4((f.get('median_pearson_r') or 0)-(r.get('median_pearson_r') or 0)) if finite(f.get('median_pearson_r')) and finite(r.get('median_pearson_r')) else None,
                'raw_expected_direction_fraction':r.get('expected_direction_fraction'),
                'filtered_expected_direction_fraction':f.get('expected_direction_fraction'),
            }
    out={
        'ok':True,'quality_status':'valid_disturbance_filtered_sensitivity_test','computed_at_local':datetime.now(TZ).isoformat(),
        'network':network.get('name'),'scene_count':len(rows),'sample_count':len(terrain),'excluded_observation_count':len(excluded),
        'warning':'Sensitivity test only. Excluded observations are algorithmically flagged candidate disturbances, not verified mowing/grazing. Compare filtered and raw results; do not automatically prefer filtered.',
        'comparison':comparison,'filtered_summary':filtered,'scenes':rows
    }
    OUT.write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    status={k:out[k] for k in ('ok','quality_status','computed_at_local','network','scene_count','sample_count','excluded_observation_count','warning','comparison')}
    status['output_file']=str(OUT.relative_to(ROOT)).replace('\\','/')
    status['next_step']='If a terrain relationship keeps the same sign and similar strength after disturbance filtering, raise confidence modestly; if it changes strongly, prioritize management/phenology as a confounder.'
