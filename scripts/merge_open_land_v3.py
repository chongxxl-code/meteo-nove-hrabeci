...
TARGET = ROOT / 'config' / 'open-land-target-candidates-v3.geojson'
REVIEW = ROOT / 'config' / 'open-land-review-v3.json'
...
def main():
    ...
    if missing:
        raise RuntimeError('Approved v3 IDs missing: ' + ', '.join(missing))

    features = []
    seen = set()
    # Preserve previously approved stable samples, but avoid duplicate v3 entries on rerun.
    ...
    out = {
        ...
        'name': 'nove-hrabeci-open-land-experimental-v2',
        'properties': {
            'status': 'stable_after_v3_visual_review',
            'generated_at_local': now,
            'review_versions': [base.get('properties', {}).get('review_version'), review.get('review_version')],
            ...
            'note': 'Network combines the seven v2 samples surviving two visual checks with seven newly approved targeted v3 samples.'
        },
        'features': features,
    }
    ...
    status = {
        ...
        'sample_count': len(features),
        'role_counts': role_counts,
        'approved_v3_labels': [x.get('label') for x in review.get('approved', [])],
        'rejected_v3_labels': [x.get('label') for x in review.get('rejected', [])],
        'missing_roles': missing_roles,
        'output_file': str(BASE.relative_to(ROOT)).replace('\\', '/'),
        'next_step': 'Recompute seasonal Sentinel NDVI/NDMI response for the expanded stable network and compare replicated terrain classes.'
    }
